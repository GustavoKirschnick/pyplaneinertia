"""
Computes a lifting-surface mass properties. A surface is built from a list of Sectionproperties,
computing its mass, center of mass, and inertia tensor about its own center of mass, in the global
frame. The Surface can be either:
...
class Surface(ABC):
    """
    Aggregates a list of Sections that form a Surface into a single set of mass properties
    (via 'combine'), and then maps them from the local coordinate frame to the global frame.
    Subclasses apply the local-to-global rotation.
    """
    def __init__(self, sections: List[SectionProperties], body_position: PositionVector, symmetric: bool):
    ...
    def properties(self) -> MassProperties:
        """
        Computes the Surface's mass properties about its center of mass in the global frame.

        Aggregates (positioned, mirror) the sections, then rotates and translates the result into
    ...
    def _positioned_sections(self):
        ...
        for section in self.sections:
            shifted = replace(section.centroid_local, y=section.centroid_local.y + offset)
    ...
    def _to_mass_properties(section: SectionProperties)-> MassProperties:
        ...
        Returns:
            The section's mass, local center of mass, and local inertia as MassProperties
        """
    ...
    def _mirror_about_y(part: MassProperties) ->MassProperties:
        ...
        The centroid's y coordinate is negated, as well as the products of inertia with y
        components.
        ...
        return MassProperties(
            mass= part.mass,
            center_of_mass=Centroid(part.center_of_mass.x, -part.center_of_mass.y, part.center_of_mass.z),
    ...
    def _rotation(self) -> np.ndarray:
        """
        Returns a 3x3 rotation matrix that maps the local coordinate frame to the global one.
...
class HorizontalSurface(Surface):
    ...
    Its local frame already matches the global one, so only the incidence is applied on the rotation.
    """
...
class VerticalSurface(Surface):
    ...
    The span (local y-axis) maps to the global Z-axis via a rotation, that also considers the incidence.
    """
