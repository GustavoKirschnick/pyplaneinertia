...
For the AirfoilCoordinates, the properties are tested against
geometric shapes (rectangle and a dislocated trapezoid) and a 
symetrical airfoil. SectionGeometry exception handling is 
tested, as well as the taper_ratio property.
...
def test_section_geometry_accepts_retangular_taper_ratio(naca0012):
    section_geometry = SectionGeometry(naca0012, chord_root=0.14, chord_tip=0.14, span= 0.3, mass= 0.3)
    assert section_geometry.taper_ratio == pytest.approx(1)

def test_section_geometry_accepts_valid_taper_ratio(naca0012):
    section_geometry = SectionGeometry(naca0012, chord_root=0.14, chord_tip=0.07, span= 0.3, mass= 0.3)
